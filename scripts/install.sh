#!/usr/bin/env bash
# AI分析师 installer — installs the CLI and optionally provisions a self-host server.
#
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/multica-ai/multica/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). AI分析师 supports macOS, Linux, and Windows." ;;
  esac
...
# ---------------------------------------------------------------------------
install_cli_brew() {
  info "Installing AI分析师 CLI via Homebrew..."
  ...
  if ! brew install "$BREW_PACKAGE" 2>/dev/null; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      ok "AI分析师 CLI already installed via Homebrew"
    else
      fail "Failed to install multica via Homebrew."
    fi
  else
    ok "AI分析师 CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing AI分析师 CLI from GitHub Releases..."
  ...
  rm -rf "$tmp_dir"
  ok "AI分析师 CLI installed to $bin_dir/multica"
}
...
upgrade_cli_brew() {
  info "Upgrading AI分析师 CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "AI分析师 CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "AI分析师 CLI is already the latest version"
  fi
}
...
install_cli() {
  if command_exists multica; then
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "AI分析师 CLI is up to date ($current_ver)"
      return 0
    fi

    info "AI分析师 CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    ok "AI分析师 CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. AI分析师 self-hosting requires Docker and Docker Compose.
...
setup_server() {
  ...
  info "Pulling official AI分析师 images..."
  pull_official_selfhost_images
  info "Starting AI分析师 services (this may take a few minutes on first run)..."
...
run_default() {
  printf "\n"
  printf "${BOLD}  AI分析师 — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ AI分析师 CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}multica setup${RESET}                # Connect to AI分析师 Cloud (multica.ai)\n"
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  AI分析师 — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ AI分析师 server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
...
run_stop() {
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No AI分析师 installation found at $INSTALL_DIR"
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the AI分析师 CLI"
