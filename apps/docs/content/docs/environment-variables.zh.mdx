---
title: 环境变量
description: self-host AI分析师 服务器需要配置的环境变量清单。
---

import { Callout } from "fumadocs-ui/components/callout";

AI分析师 的 [自部署](/self-host-quickstart) 服务器启动时从环境变量读取配置——数据库、登录、邮件、存储、注册白名单都在这里配。这一页按用途分组给完整清单：每组说清楚**不设会怎样**、**生产必须设哪几个**。Auth 相关那几个怎么真正配见 [登录与注册配置](/auth-setup)。
...
AI分析师 支持两种邮件发送通道——[Resend](https://resend.com/) 适合公网部署，SMTP relay 适合内网/自部署。同时设置时 `SMTP_HOST` 优先级高于 `RESEND_API_KEY`。
...
AI分析师 存储用户上传的附件（评论里的图片、文件等）。**优先走 S3**；不配 S3 就回落本地磁盘。
...
**邀请流程本身不检查 signup 白名单**——但被邀请人必须先能**登录**才能接受邀请。如果对方已经有 AI分析师 账号（比如在其他工作区注册过），可以直接接受，不受白名单影响；**如果对方还没注册过**，他们登录的第一步（发送验证码）仍然会过白名单检查，被 `ALLOW_SIGNUP=false` 或 `ALLOWED_EMAILS` / `ALLOWED_EMAIL_DOMAINS` 拒绝的邮箱**无法完成注册，也就没法接受邀请**。
...
- `/api/webhooks/github` 直接返回 **`503 github webhooks not configured`**——secret 没配置时 AI分析师 拒绝处理任何 webhook 事件，而不是把所有签名当 valid
...
默认上报到 AI分析师 官方 PostHog 实例。不想上报就把 `ANALYTICS_DISABLED=true`。
