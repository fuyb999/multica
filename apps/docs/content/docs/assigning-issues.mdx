...
Assign an [issue](/issues) to an [agent](/agents) and it works as the **official assignee** until the work is done — it can read the full issue context (description + all [comments](/comments)) and change status, post comments, and edit fields. This is the **most common and heaviest** of AI分析师's four trigger paths. The same flow also accepts a [squad](/squads) as the assignee — AI分析师 then triggers the squad's **leader agent** instead.
...
When a non-Backlog issue is assigned to an agent, AI分析师 immediately does the following in the background:
...
**If the agent is offline**, the `task` waits in the queue — **it times out and fails after 5 minutes** with reason `runtime_offline`. For retryable sources (assign, @-mention, chat), AI分析师 automatically re-enqueues it. See [**Tasks**](/tasks) for the full retry rules.

Assigning also auto-subscribes the agent to the issue — but in AI分析师 **agents do not receive inbox notifications** (only members do). This subscription is internal bookkeeping with no user-visible side effect.
