...
description: AI分析师's core unit of work — assignable to a person or to an agent.
---

import { Callout } from "fumadocs-ui/components/callout";

An issue is a self-contained unit of work in AI分析师 — a bug, a new feature, a thing that needs doing. Every issue has a **title**, a **description** (Markdown supported), a **status**, a **priority**, an **assignee**, and optionally belongs to a **project**. If you've used Linear or Jira, this is the same shape.

**AI分析师's defining trait is that an issue's assignee can be a person or an [agent](/agents)** — which is where we'll start.
...
AI分析师 has seven statuses. **Any status can move directly to any other** — AI分析师 doesn't impose a workflow, and won't stop you from jumping from `backlog` straight to `done`.
