...
export const PROJECT = {
  ...
  description:
    "A few things to try in AI分析师. Work through them at your own pace.",
};
...
export function buildWelcomeIssueText(
  ...
  const header = [
    ...
    ``,
    `[Learn how AI分析师 works →](https://multica.ai/docs/how-multica-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  const sharedInstructions = [
    ...
    `2. **Explain how we work together in AI分析师**:`,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 Welcome to AI分析师 — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AI分析师. They plan to use you mostly for **coding work**.\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 Welcome to AI分析师 — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AI分析师. They want your help with **planning and breaking down work**.\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 Welcome to AI分析师 — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AI分析师. They'll use you for **research and writing** — drafting, summarizing, analysis.\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 Welcome to AI分析师 — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AI分析师. They're **exploring** what AI分析师 can do — no specific goal yet.\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 Welcome to AI分析师 — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AI分析师. ${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 Welcome to AI分析师 — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AI分析师.\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**Every issue has a right-side panel** called **Properties**. From there you control who works on what. Agents in AI分析师 are triggered when an issue has:`,
        ``,
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        `AI分析师 works best when a small team shares agents.`,
        ``,
        `**Where to find it**:`,
